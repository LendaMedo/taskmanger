...
require_once '../functions.php'; // Adjust path
require_once '../auth/session.php'; // Adjust path
            ...
            if (!empty($errors)) {
                ...
                $updated = updateRecord('Users', $updateData, 'id = ?', [$userIdToUpdate]);
                 if ($updated) {
                    ...
                    $response = ['success' => false, 'message' => 'Failed to update user or no changes made.'];
                }
            } else {
                $response = ['success' => false, 'message' => 'No data provided for update.'];
            }
